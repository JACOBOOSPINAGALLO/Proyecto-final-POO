...
class Inventario:
    ...
    def actualizar_producto(self, producto_id, nueva_cantidad, nuevo_precio, actor_id):
        ...
        if nueva_cantidad is not None:
            updates.append("cantidad = ?"); params.append(nueva_cantidad)
        if nuevo_precio  is not None:
            updates.append("precio = ?");   params.append(nuevo_precio)
        if not updates:
            print("No se especificó cantidad o precio para actualizar"); return False

        params.append(producto_id)
        sql = f"UPDATE productos SET {', '.join(updates)} WHERE id = ?"
        with self._conect() as c:
            cur = c.cursor()
            cur.execute(sql, tuple(params)); c.commit()
    ...
    def eliminar_producto(self, producto_id, actor_id):
        with self._conect() as c:
            cur = c.cursor()
            cur.execute("DELETE FROM productos WHERE id = ?", (producto_id,)); c.commit()
            success = cur.rowcount > 0
    ...
    def actualizar_empleado(self, empleado_id, nuevo_cargo, nuevo_salario, actor_id):
        ...
        if nuevo_cargo is not None:
            updates.append("cargo = ?"); params.append(nuevo_cargo)
        if nuevo_salario is not None:
            updates.append("salario = ?"); params.append(nuevo_salario)
        if not updates:
            print("No se especificó cargo o salario para actualizar."); return False
    ...
    def eliminar_empleado(self, empleado_id, actor_id):
        with self._conect() as c:
            cur = c.cursor()
            cur.execute("DELETE FROM empleados WHERE id = ?", (empleado_id,)); c.commit()
            success = cur.rowcount > 0
...
def obtener_entrada(mensaje, tipo=str, obligatorio=True):
    while True:
        ...
        if tipo is int:
            ...
            except ValueError: print("Debes ingresar un valor numérico.")
        else:
            return val
