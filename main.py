...
class Empleado: 
    ...
    def __str__(self): 
        return ( 
            ...
            f"Contraseña: {self.__password}" # Considera no mostrar la contraseña en __str__ por seguridad 
        ) 
...
class Inventario: 
    def __init__(self, db_nombre="inventario.db"): 
        ...
        # Unir la ruta del directorio del script con el nombre del archivo de la base de datos 
        self.db_nombre = os.path.join(script_dir, db_nombre) 
...
if __name__ == "__main__": 
    ...
    while not current: 
        ...
        user = inv.buscar_empleado_por_id(eid) # No necesita actor_id para buscarse a sí mismo al loguear 
        if not user: 
            print("ID inválido. Intenta de nuevo.") 
            continue 
 
        # Vamos a asumir que la contraseña almacenada es la que se debe usar: 
        pwd_ingresada = input("Contraseña: ").strip() 
        # Para acceder a __password desde fuera, Python hace "name mangling" a Clase__atributo 
        if user._Empleado__password == pwd_ingresada: # Esto compara con la contraseña almacenada 
            current = user 
        else: 
    ...
    while True: 
        if current.cargo.lower() != "gerente": 
            ...
            if opcion == "1": 
                ...
                nc_val = int(nc) if nc else None 
                np_val = float(np) if np else None 
                inv.actualizar_producto(pid, nc_val, np_val, actor_id=current.id) 
            ...
            # Empleados 
                ...
                # Aquí, al agregar empleado, la contraseña que le pidas es la que se guardará. 
                # Si quieres que sea ID+420, tendrías que obtener el ID primero o generarla DESPUÉS 
                # de insertar y obtener el lastrowid, y luego hacer un UPDATE. 
                # Es más simple que el gerente establezca una contraseña directamente. 
                ...
                ns_str = input("Nuevo salario (dejar vacío): ").strip() 
                ns_val = float(ns_str) if ns_str else None 
                inv.actualizar_empleado(eid, nc, ns_val, actor_id=current.id) 
